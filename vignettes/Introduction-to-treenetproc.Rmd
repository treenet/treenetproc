...
<br><br>
```{r Figure 1, fig.asp = 0.35, fig.retina = 2, out.width = "80%", echo = FALSE}
library(treenetproc)
data("dendro_data_L1")
data("dendro_data_L2")
...
minutes).  
How the time alignement works is documented in 
...
```{r processs to L2, fig.show="hide", message=FALSE, warning=FALSE, results="hide"}
# Clean time-aligned (L1) dendrometer data and plot changes
proc_dendro_L2(dendro_data = dendro_data_L1, plot_period = "monthly",
               plot_export = FALSE)
```

```{r plot L2 data first_plot, fig.asp = 0.9, echo = FALSE, fig.retina = 2, fig.keep = "first", results = "hide"}
proc_dendro_L2(dendro_data = dendro_data_L1, plot_period = "monthly", 
               plot_export = FALSE)
```
<br>
...
growth curves aligned by the day of year. Beneath the plot all input values as
well as the package version used for data cleaning are printed.


```{r plot L2 data second_plot, fig.asp = 1.5, echo = FALSE, fig.retina = 2, fig.keep = "last", results = "hide", message = FALSE}
proc_dendro_L2(dendro_data = dendro_data_L1, plot_period = "monthly", 
               plot_export = FALSE)
```
...
```{r shrinking example plot, fig.asp = 0.5, echo=FALSE, fig.retina = 2}
data("dendro_data_L1")
data_L1 <- dendro_data_L1 %>% 
  dplyr::filter(series == "site-1_dendro-2")
...
Such problems can be fixed by increasing the values of `tol_jump` and `tol_out`.

<br>
```{r shrinking example higher tol code, fig.show="hide", results="hide", message = FALSE, warning = FALSE}
...
```{r shrinking example higher tol plot, fig.asp = 0.5, echo=FALSE, fig.retina=2}
par(mar = c(2.1, 4, 1, 2))
...
```{r frost no temp data_2, fig.asp = 0.5, echo=FALSE, results="hide", fig.retina = 2, warning=FALSE, message=FALSE}
# load data
data("dendro_data_L2")
# subset data for example
...
```{r frost with temp data_2, fig.asp = 0.5, echo=FALSE, results="hide", fig.retina = 2, warning=FALSE, message=FALSE}
# load data
data("temp_data_L1")
...
```{r corr_dendro_L3 example_1, fig.asp = 0.9, results="hide", fig.retina=2, fig.keep = "first", message=FALSE, warning=FALSE}
# load data
data("dendro_data_L1")
# subset data for example
data_L1 <- dendro_data_L1 %>% 
...
1. Corrections `1-4` and `6` are not necessary since the raw data seems to be
   correct
2. A jump occurring on August 13 is not corrected
3. Not all erroneous data is removed at the beginning (August 1-5)
<br>
...
```{r corr_dendro_L3 example_3, fig.asp = 0.9, results="hide", fig.retina=2, fig.keep = "first", message=FALSE, warning=FALSE}
