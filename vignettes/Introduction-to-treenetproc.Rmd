...
cleaning process (number of iterations specified in `iter_clean`) the changes
occurred:
...
The thresholds are printed for each iteration in the data cleaning process 
(number of iterations is specified in `iter_clean`).
...
# clean dendrometer data (tol = 10, default)
proc_dendro_L2(dendro_data = data_L1, tol = 10, plot_export = FALSE)
```
