...
to regular time intervals (level 1: `L1`) and cleaned (level 2: `L2`).
...
`proc_dendro_L2`. To increase the quality of the cleaning process,
...
package version used for data cleaning are reported.
...
If values are deleted (i.e. classified as outliers) that are not outliers, the
...
After data cleaning and the manual removal of remaining errors, the package
offers two functions to calculate additional physiological parameters that may
be of use for later analyses. The function `grow_seas` returns the day of year 
of growth onset and growth cessation. The function `phase_stats` calculates 
calculates the timing, duration, amplitude and the rate of change of shrinkage
