...
Alternatively the data can be provided in wide format with data series in different columns (Table 2).
<br><br>
...
* **gro_yr**: growth since the beginning of the year (calculation of yearly
  growth according to the zero growth concept, Zweifel et al. 2016)
...
```{r processs to L2, fig.asp = 0.9, fig.show="hide", results="hide", message=FALSE, warning=FALSE}
# Clean time-aligned (L1) dendrometer data and plot changes
proc_dendro_L2(dendro_data = dendro_data_L1, plot_period = "monthly",
               plot_export = FALSE)
```

```{r plot L2 data first_plot, fig.asp = 0.9, fig.retina = 2, fig.keep = "first", echo = FALSE, results = "hide"}
proc_dendro_L2(dendro_data = dendro_data_L1, plot_period = "monthly", 
               plot_export = FALSE)
```
<br>
...
growth curves aligned by the day of year. The first year is shown as a dashed
line since growth starts as soon as the measurements start. Beneath the plot 
all input values as well as the package version used for data cleaning are 
printed.


```{r plot L2 data second_plot, fig.asp = 1.4, fig.retina = 2, fig.keep = "last", echo = FALSE, results = "hide", message = FALSE}
proc_dendro_L2(dendro_data = dendro_data_L1, plot_period = "monthly", 
               plot_export = FALSE)
```
...
proc_dendro_L2(dendro_data = data_L1, tol_jump = 1, tol_out = 10, 
               plot_export = FALSE)
```
...
the function `corr_dendro_L3()`. There are three possibilities to revert 
wrong changes:
...
```{r corr_dendro_L3 example_1, fig.asp = 0.9, fig.keep = "first",  fig.retina=2, results="hide", message=FALSE, warning=FALSE}
# load data
data("dendro_data_L1")
# subset data for example
data_L1 <- dendro_data_L1 %>% 
...
data_L2 <- proc_dendro_L2(dendro_data = data_L1, plot_period = "monthly", 
                          plot_export = FALSE)
```
<br><br>
...
1. Corrections `74` is not necessary since the raw data seems to be correct
2. A jump occurring on August 13 is not corrected
3. Not all erroneous data is removed at the beginning (August 1-6)
<br>

Correct the above-described errors:


```{r corr_dendro_L3 example_3, fig.asp = 0.9, fig.keep = "first", fig.retina=2, results="hide", message=FALSE, warning=FALSE}
# correct described errors
corr_dendro_L3(data_L1 = data_L1, data_L2 = data_L2, 
               remove = 74, 
               force = "2013-08-12", 
