...
(level 1: `L1`) and then cleaned (level 2: `L2`). This vignette demonstrates
the workflow of the package with sample code. For more information on the 
functionality of the package, the user is referred to Knüsel et al. (2020, in
prep).
<br><br>
...
of the cleaning process (cf. Step 2: Providing temperature data).

The raw dendrometer or climate data can be provided in long (Table 1) or wide
format (Table 2). In both cases the data has to contain a timestamp column
named `ts`. The timestamp can be in any standard date format (default is 
`date_format = "%Y-%m-%d %H:%M:%S"`). Data of multiple sensors can either be
specified in a column named `series` to separate the sensors (long format, 
Table 1) or as separate columns (wide format, Table 2). Dendrometer and 
temperature data can also be provided in the same `data.frame`. In this case,
the name of the temperature data series has to contain the string '`temp`' 
(cf. Table 2).
<br><br>
...
minutes). Temperature data can be time-aligned in the same way as dendrometer
data. In case data is provided in `wide` format, this has to be specified
...
# Step 2: Data cleaning (L2)
Time-aligned dendrometer data can be cleaned with the function 
`proc_dendro_L2`. Time-aligned temperature data can optionally be provided 
along with dendrometer data to `temp_L1`.
...
package version used for data cleaning are printed.
...
## Adjustments to data cleaning parameters
...
An increase in the value of the variable `tol_out` makes sense if values are
deleted (i.e. classified as outliers) that are not outliers. A  decrease, in
contrast, is appropriate if outliers are not detected as such.
...
par(mar = c(2.1, 4, 1, 2))
plot(data = dendro_L2, value ~ ts, type = "n", ylab = "L2", 
     las = 1)
lines(data = dendro_L1, value ~ ts, col = "grey70")
...
detection in periods of probable frost (i.e. temperature < `lowtemp`), which 
are indicated with a horizontal light-blue line in the plot.
...
the function `corr_dendro_L2()`. There are three possibilities to reverse 
wrong changes:

* **reverse**: specify the numbers of the changes that should be reversed.
...
reflected in the returned `data.frame` and all changes are added to the column
`flags` as:
...
After data cleaning and the removal of remaining errors, additional functions
can be used to calculate the start and end of the growing season as well as
several characteristics of shrinkage and expansion phases.
<br>


### Growing season
The start and end of the growing season can be calculated with the function 
`grow_seas`. 
...
knitr::kable(grow_seas, caption = "**Table 4**: Growth start and 
             end.")
```
<br><br>
...
knitr::kable(phase_stats[1:5, ], caption = "**Table 5**: Characteristics of 
             shrinkage (`shrink`) and expansion (`exp`) phases.")
```
<br>
...
# References
Knüsel S, Haeni M, Wilhelm M, Peters R, Zweifel R (2020) treenetproc: An R
package to clean, process and visualise dendrometer data. In prep.
